...
unsigned int NUM_TWEETS = 3096327;
...
void    PrintPair      ( Tweet a, Tweet b, unsigned int time );
Tweet * ReadTweetsFile ( char * filename, unsigned int * numTweets );
...
int main (int argc, char * argv[])
{
    ...
    {
        unsigned int j;
        unsigned int timei = t[i].timestamp,
            useri = t[i].id;

        for (j=i-1; j>0; --j) 
        {
            unsigned int userj = t[j].id,
                timej = t[j].timestamp;

            unsigned int timediff = abs(timej-timei);
            if (timediff <= timespan) {
                ...
                if (userj != useri) {
                    ...
                    if (d <= max_distance) {
                        PrintPair(t[i], t[j], abs(timei+timej)/2);
                    }
                }
...
{
    unsigned int nRadius = 6371; // Earth's radius in Kilometers
