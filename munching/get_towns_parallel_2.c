...
int main (int argc, char ** argv) 
{
	...
	for (i=0; i<num_tweets; i++) {
		...
		float best_diff_lat = FLT_MAX, best_diff_lon = FLT_MAX;

		for (j=0; j<num_towns; j++) {

			float diff_lat = fabs(tweets.lat[i] - towns.lat[j]);
			float diff_lon = fabs(tweets.lon[i] - towns.lon[j]);

			// first check if the difference in latitude AND in longitude is higher already, there is nothing to see
			if ((diff_lon < best_diff_lon) || (diff_lat < best_diff_lat))
			{
				...
				{ 
					...
					min_idx = j; 
					best_diff_lat = diff_lat;
					best_diff_lon = diff_lon;
				}
			}
		}
...
double haversine_nounit(double long1, double lat1, double long2, double lat2)
{
	...
	double dlat  = ( lat2 -  lat1) * DEG_2_RAD;
	double a     = pow(sin(dlat/2.0), 2) + cos(lat1*DEG_2_RAD) * cos(lat2*DEG_2_RAD) * pow(sin(dlong/2.0), 2);
	// since atan and product-times-a-constant are order-preserving operations, I can stop here
	return a;
}
